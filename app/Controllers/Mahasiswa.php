...
class Mahasiswa extends BaseController
{
    ...
    public function simpan_tambah_identitas_mhs($id_peserta)
    {
        ...
        if (!$this->validate([
            'no_induk'    => [
                'rules' => 'required|is_unique[identitas.no_induk]',
                'errors' => [
    ...
    public function simpan_tambah_lampiran_mhs($no_induk)
    {
        ...
        // if ($scan_prestasi['prestasi_1']->getError() != 4) {
            ...
            //         'scan_akreditasi_pt' => ['mime_in' => 'Scan prestasi harus berupa file PDF'],
    ...
    public function simpan_edit_mhs()
    {
        /*******************    IDENTITAS    ********************/
        $identitas = $this->MIdentitas->find_identitas_user(user_id())->getFirstRow('array');
        ...
        if ($identitas['no_induk'] != $input_no_induk) {
            if (!$this->validate([
                'no_induk'    => [
                    'rules' => 'requiredrequired|numeric|is_unique[identitas.no_induk]',
                    'errors' => [
                        'is_unique' => 'NIK sudah digunakan',
                    ]
                ],

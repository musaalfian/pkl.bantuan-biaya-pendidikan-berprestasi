...
<div class="py40 bglight2 pengumuman" id="pengumuman">
    <div class="container">
        ...
        // kondisi jika tanggal sekarang kurang dari tanggal seleksi administrasi
        if ($tanggal_sekarang < $tanggal_seleksi_administrasi) {
            $status_pendaftaran['id_status_pendaftaran'] = 4;
            $status_pendaftaran['nama_status'] = 'Proses Verifikasi Data';
        }
        ?>
        <!-- mengecek apakah status final sudah ada? -->
        <?php if (
            $status_final == null || $tanggal_sekarang < $tanggal_pengumuman
        ) { ?>
        <div class="bgwhite bdblue br25 p-4 p-sm-5 content">
            ...
            <h4 class="mt-3 biru fs16">Pengumuman akan disampaikan pada tanggal :
                <strong><?= date_format(date_create($tanggal_pengumuman), 'd-m-Y'); ?></strong>
                <?php elseif ($status_pendaftaran['id_status_pendaftaran'] == 2) : ?>
                <h3 class="text-red fs20">Mohon Maaf!</h3>
                <h4 class="mt-3 lightgrey fs16">Anda tidak memenuhi syarat pendaftaran bantuan biaya pendidikan
                    berprestasi.</h4>
                <?php else : ?>
                ...
                <?php if ($tanggal_sekarang > $tanggal_penutupan_pemberkasan) : ?>
                <h4 class="mt-3 biru fs16">Pengumuman akan disampaikan pada tanggal :
                    <strong><?= date_format(date_create($tanggal_pengumuman), 'd-m-Y'); ?></strong>
                </h4>
                <?php elseif ($tanggal_sekarang <= $tanggal_penutupan_seleksi_administrasi) : ?>
                <h4 class="mt-3 biru fs16">Seleksi administrasi akan dilaksanakan sampai dengan tanggal :
                    <strong>
                        <?= date_format(date_create($tanggal_penutupan_seleksi_administrasi), 'd-m-Y'); ?></strong>
                </h4>
                <?php elseif ($tanggal_sekarang <= $tanggal_penutupan_pemberkasan) : ?>
                <h4 class="mt-3 biru fs16">Pemberkasan Teknis akan dilaksanakan sampai dengan tanggal :
                    <strong>
                        <?= date_format(date_create($tanggal_penutupan_pemberkasan), 'd-m-Y'); ?></strong>
                </h4>
                <?php endif; ?>
                <?php endif; ?>
                <div
                ...
                <div class="mt-3">
                    <!-- btn edit data -->
                    <?php if ($identitas['pesan'] != null && $tanggal_sekarang >= $tanggal_seleksi_administrasi) : ?>
                    <p class="fs16 fw-bold mb-2">Alasan :</p>
                    <div class="alert alert-primary bg-transparent" role="alert">
                        <?= $identitas['pesan']; ?>
                    </div>
                    <?php endif ?>
                    <?php if ($identitas['id_status_pendaftaran'] == 3 && $tanggal_sekarang >= $tanggal_seleksi_administrasi && $tanggal_sekarang <= $tanggal_penutupan_pemberkasan) { ?>
                    <a href="<?= base_url(); ?>/pendaftaran/edit_pendaftaran"
        ...
        <?php if (
            $tanggal_sekarang >= $tanggal_pengumuman
        ) { ?>
        ...
        <div class="mt40 bdblue bgwhite br25 p-4 p-lg-5 laporan__dana">
            ...
            <h4 class="fs20 bold blue">Terima kasih telah mengirim laporan rencana penggunaan dan informasi rekening.
            </h4>
            ...
            <div class="text-center">
                <div class="icon mx-auto">
                    <img src="<?= base_url('assets/img/icon-transfer-berhasil.svg'); ?>" width="100%" height="100%"
                        alt="">
                </div>
                <h4 class="bold black mt-4 fs18">Transaksi Berhasil!</h3>
                    <h4 class="mt-2 lightgrey fs16 w-100 mx-auto desc__berhasil">Dana bantuan biaya pendidikan
                        berprestasi telah dikirim dengan nominal sebesar <span class="bold blue">Rp
                            <?= number_format($identitas['nominal'], 2, ',', '.'); ?></span>. Silahkan periksa dan
                        gunakan sebaik mungkin.</h4>
            </div>
            <?php endif ?>
            ...
            <div class="mt-3 mb-3 w-100 content">
                ...
                <div class="">
                    ...
                    <div class="alert alert-primary mt-3">
                        ...
                        <p>3. Scan rekening tidak lebih dari 2MB dengan format penamaan file :
                            <strong>(nik)_scan_bpd.pdf</strong>
                        </p>
                    </div>
                    ...
                    <form action="<?= base_url(); ?>/penerima/simpan_tambah_laporan/<?= $identitas['no_induk']; ?>"
                        ...
                        <div class="alert alert-primary mt-3 mb-0">
                            <h5 class="bold fs14 mb-2">Ketentuan :</h5>
                            <p>1. Silahkan unduh instrumen penggunaan dana <a
                                    href="<?= base_url(); ?>/assets/informasi/file/instrumen_beasiswa.docx"
                                    class="fs14 text-decoration-underline">Disini</a></p>
                            <p>2. Instrumen penggunaan dana diisi dan diunggah melalui form dibawah ini</p>
                            <p>3. Untuk format penamaan file : (nik)_scan_laporan, Contoh: 240601191_scan_laporan
                            </p>
                            <p>4. Ukuran file tidak lebih dari 5 MB</p>
                        </div> <br>
                        <div class="">
                            <label for="laporan" class="form-label fs14 bold">Scan Laporan Penggunaan Dana <span
                                    class="text-red">*</span></label>

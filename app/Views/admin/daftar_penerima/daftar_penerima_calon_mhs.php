...
<div class=" bg-abu p40">
    <div class=" admin-content mx-auto">
        ...
        <div class="p-4 br20 bg-white mt20 ">
            <p class="fw-bold">Edit status pembayaran keseluruhan : <a href="" data-bs-toggle="modal" data-bs-target="#ubahStatusPembayaranKeseluruhan">
                    Ubah
                </a></p>
...
<!-- modal status PEmbayaran pendaftar -->
<?php foreach ($identitas as $data) : ?>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form action="<?php base_url() ?>/admin_daftar_penerima/ubah_status_pembayaran/<?= $data['no_induk'] ?>/2" method="POST">
                        <!-- <select class="form-select mt-2" id="statusPembayaran" name="statusPembayaran">
                        <option selected hidden>---</option> -->
                        <div class="form-check">

                            <?php foreach ($status_pembayaran as $status_pembayarans) : ?>
                                <input type="radio" id="statusPembayaran_<?= $status_pembayarans['id_status_pembayaran']; ?>" class="form-check-input" <?= ($status_pembayarans['id_status_pembayaran'] == $data['id_status_pembayaran']) ? 'checked' : ''; ?> name="statusPembayaran" value="<?= $status_pembayarans['id_status_pembayaran']; ?>">
                                <label class="form-check-label" for="statusPembayaran_<?= $status_pembayarans['id_status_pembayaran']; ?>">
                                    <?= $status_pembayarans['nama_status_pembayaran']; ?>
                                </label>
                            <?php endforeach ?>
                        </div>
    ...
    <div class="modal fade" id="ubahNorek_<?= $datas['no_induk']; ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="<?php base_url() ?>/admin_daftar_penerima/ubah_nomor_rekening/<?= $datas['no_induk'] ?>/2" method="POST">
                        <p class="bold">Nomor Rekening saat ini : <?= $datas['no_rek']; ?></p>
                        <label for="" class="mt-2">Ubah Nomor rekening</label>
                        <input value="<?= $datas['no_rek']; ?>" type="number" class="border w-100" name="nomorRekening">
                        <div class="modal-footer">
                            <button type="button" class="btn btn-danger" data-bs-dismiss="modal">Close</button>
                            <button type="submit" class="btn btn-primary">Simpan</button>
                        </div>
    ...
    <div class="modal fade" id="ubahNominal_<?= $datas['no_induk']; ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="<?php base_url() ?>/admin_daftar_penerima/ubah_nominal/<?= $datas['no_induk'] ?>/2" method="POST">

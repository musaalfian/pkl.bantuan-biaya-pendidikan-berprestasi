  ...
  <div class="awal row me-0">
    <div class="awal__content login col-md-6 col-12 bgwhite d-flex justify-content-center align-items-center">
      <div class="py-5 px-3 px-sm-5 px-md-0">
        ...
        <div class="content">
          <form action="<?= route_to('register') ?>/admin" method="post">
            <?= csrf_field() ?>
            <div class="mt40">
              <label for="email">Email</label>
              <input class=" form-control mt-2 <?php if (session('errors.email')) : ?>is-invalid<?php endif ?>"
                placeholder="Masukan email" autofocus type="email" id="email" name="email" aria-describedby="emailHelp"
                value="<?= old('email') ?>" />
            </div>
            <div class="mt-3">
              <label for="username">Nama pengguna</label>
              <input class="form-control mt-2 <?php if (session('errors.username')) : ?>is-invalid<?php endif ?>"
                placeholder="Masukan nama pengguna" type="text" id="username" name="username"
                value="<?= old('username') ?>" />
            </div>
            <div class="mt-3">
              <label for="sandi">Kata Sandi</label>
              <p class="blue">Tips : gunakan kombinasi huruf, angka, dan karakter untuk membuat kata sandi yang kuat.
              </p>
              <div class="input__sandi position-relative mt-2">
                <input class="form-control <?php if (session('errors.password')) : ?>is-invalid<?php endif ?>"
                  autocomplete="off" placeholder="Masukan kata sandi" type="password" id="sandi" name="password"
                  autocomplete="off" />
                <span class="eye position-absolute"><i class="bi bi-eye-fill" area-hidden="true" onclick="toogle()"
                    id="eye"></i></span>
              </div>
            ...
            <div class="mt-3">
              ...
              <div class="input__sandi position-relative mt-2">
                <input class="form-control <?php if (session('errors.pass_confirm')) : ?>is-invalid<?php endif ?>"
                  autocomplete="off" placeholder="Ulangi kata sandi" type="password" id="ulangiSandi"
                  name="pass_confirm" autocomplete="off" />
                <span class="eye2 position-absolute"><i class="bi bi-eye-fill" area-hidden="true" onclick="toogle2()"
                    id="eye2"></i></span>
              </div>
            </div>
            ...
            <div class="daftar__button mt40">
              <button type="submit" id="submit_login"
                class="btn btn-primary shadow-none fs14 btn__blue w-100">Daftar</button>
            </div>
          </form>
          <div class="mt40">
            <p class="fs16">
